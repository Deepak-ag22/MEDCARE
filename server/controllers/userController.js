    ...
    try {
        ...
        req.login(newUser, (err) => {
            if (err) {
                ...
                return res.status(201).json({ message: "User created but auto-login failed", user: newUser, ok: true });
            }
            return res.status(201).json({ message: "User created and logged in", user: newUser, ok: true });
        });
    } catch (error) {
...
const loginUser = (req, res, next) => {
    passport_local.authenticate("local", (err, user, info) => {
        ...
        if (!user) {
            return res.status(401).json({ message: info.message || "Authentication failed", ok: false });
        }
        req.logIn(user, (err) => {
            if (err) {
                return res.status(500).json({ message: "Login failed", ok: false });
            }
